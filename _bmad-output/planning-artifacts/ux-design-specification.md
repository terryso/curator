...
Curator 是一款原生 macOS 照片管理 Agent，用自然语言指令替代整个专业照片工具链。用户通过描述需求来管理照片图库——AI Agent 负责规划工作、透明执行，并在关键操作前等待确认。这不是一个碰巧处理照片的聊天应用，而是一个 Agent 工作空间，核心范式从"用户操作工具"转变为"用户表达意图，Agent 自主执行"。
...
- **UI 框架：** SwiftUI 为主体，AppKit 互操作用于 PhotoKit 和系统集成
- **离线：** 本地操作（浏览、感知哈希、元数据分析）支持离线；LLM 功能需网络
- **系统能力：** PhotoKit 深度集成、Finder 拖放、通知中心、菜单栏图标（MVP 后）
...
2. **一次性权限轰炸。** 不要在首次启动时要求照片库完全访问 + 通知 + 位置等所有权限。按需请求，渐进授权。
...
4. **Swift 原生。** 与 OpenAgentSDKSwift 和 PhotoKit 同一技术栈，无需桥接或封装。
...
**期望的魔法时刻：** 输入"找出重复照片"后，60 秒内看到 Agent 在真正扫描和分析自己的照片库——不是预设结果，是实时的、针对自己照片的分析。
...
**2. Agent 规划（Planning）：**
- Agent 展示执行计划卡片："我将分 3 步完成：1. 扫描照片库 2. 分析相似度 3. 确认重复。预计 5 分钟。开始？"
- 用户看到计划后点击"开始"——明确授权
