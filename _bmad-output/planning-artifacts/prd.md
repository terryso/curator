...
editHistory:
  - date: '2026-04-19'
    changes: 'MVP 照片来源策略从 PhotoKit 切换为本地文件夹；PhotoKit 移至第二阶段；FR1-FR7 重写为来源无关；领域需求、风险表、NFR 同步更新'
---
...
一款原生 macOS 应用，用自然语言指令替代整个专业照片工具链。用户通过描述自己的需求来管理、整理、编辑和处理照片——AI Agent 负责规划工作、透明执行，并在关键操作前等待用户确认。照片来源可插拔：MVP 支持本地文件夹，后续扩展 Apple 照片图库和云存储。

**目标用户:** 拥有大量照片（1000+ 张）的 Mac 用户，无论是散落在文件夹中还是存储在系统相册，都认为现有工具无法满足智能整理、去重和批量处理需求。他们想要专业级的效果，但不想学习专业级工具。

**问题:** 照片越来越多，而管理工具要么太基础（Apple 照片、Finder 手动整理），要么太复杂（Lightroom、Photoshop）。用户积累了成千上万张照片，却没有实用的方法来批量整理、去重、重命名或增强。现有方案如 PowerPhotos（$30）、PhotoSweeper 和 Eagle 依赖规则匹配——没有 AI、不理解照片内容、不支持自然语言交互。
...
**竞争定位:**
- vs Apple 照片/Finder：AI 智能化 vs 基础整理
- vs PowerPhotos/PhotoSweeper/Eagle：Agent 驱动 vs 规则驱动
...
| **领域** | 消费级 AI / 照片管理 |
| **复杂度** | 中等——AI 基础设施由 SDK 处理；复杂性在于多来源文件系统集成、图像分析管线和用户信任 |
...
- **照片来源集成:** 正确读写本地文件夹照片，无数据损坏；Repository 协议验证可插拔性（文件夹来源可替换为其他来源）
...
**第一步——首次启动:**
- 系统请求选择照片文件夹 → 小张选择照片目录 → 文件夹访问通过 security-scoped bookmark 持久化
...
**高潮:** 3 分钟后，34 组重复全部处理完毕。小张的照片第一次感觉"干净了"。信任开始建立。

**结局:** 小张输入"重命名去年国庆的照片"——又一次成功。决定每周使用。

**所需能力:** 文件夹读取访问、自然语言解析、智能去重引擎、AI 视觉相似度分析、实时进度展示、批量操作预览和审批
...
**第一步——谨慎开始:**
- 老王看到文件夹访问请求——比系统相册权限没那么可怕，但还是谨慎
...
- **Security-scoped bookmark:** 用户选择文件夹后，通过 security-scoped bookmark 持久化访问权限；应用重启后无需重新授权
- **沙盒边界:** 应用必须在 macOS 沙盒约束内操作；仅访问用户明确授权的文件夹
- **EXIF 元数据:** 通过 ImageIO 框架读取照片 EXIF 数据（日期、相机型号、GPS 等）；部分元数据可能缺失
- **大规模性能:** 10,000+ 文件的目录扫描需要异步分页和后台处理

### PhotoKit API 约束（MVP 后参考）

- **默认只读:** 应用以只读权限启动；写操作需要用户明确升级
- **元数据限制:** 部分照片元数据字段通过 PhotoKit 是只读的；应用需优雅处理
- **图库监控:** PHPhotoLibraryChangeObserver 检测外部图库变更
...
- **绝不修改原始文件:** Agent 操作照片的元数据（文件名、目录结构），绝不覆盖原始图像文件
...
- **强化运行时:** 兼容强化运行时要求
- **权限声明:** 正确声明文件访问书签权限（`com.apple.security.files.user-selected.read-write`）；`com.apple.security.personal-information.photos` 保留用于 MVP 后 PhotoKit 支持
...
**市场空白:** 没有产品同时具备 AI Agent 能力 + 多来源可插拔 + 自然语言界面 + 透明执行。
...
*本地文件夹（MVP）:*
- 读取：浏览用户选择文件夹中的照片，支持常见格式（JPEG、PNG、HEIC、TIFF、RAW）
- 写入：重命名文件、移动文件到子目录、修改文件元数据
- 资源管理：通过 FileManager 访问全分辨率图像用于 AI 分析
- 变更监控：通过 DispatchSourceFileSystemObject 监控文件夹变更
- 文件夹选择：通过 NSOpenPanel 让用户选择照片文件夹，security-scoped bookmark 持久化

*Apple 照片图库（MVP 后）:*
- 读取：浏览照片图库、相册、智能相册、文件夹
- 写入：创建/删除相册、修改照片元数据（标题、描述、关键词）、在相册间移动照片
- 资源管理：访问全分辨率图像和缩略图用于 AI 分析
- 图库监控：通过 PHPhotoLibraryChangeObserver 检测外部图库变更（如从 iPhone 同步的新照片）

**macOS 原生功能:**
- 文件夹选择器：NSOpenPanel 让用户选择照片目录，支持拖放文件夹
...
**核心假设:** 自然语言 + AI Agent 比传统工具链更高效地管理照片。
...
**第二阶段（增长——第 4-6 个月）:**
- Apple 照片图库（PhotoKit）来源支持——浏览、读写系统相册
- 按事件/主题/人物创建智能相册
- 照片质量检测（模糊、过曝）
- 批量编辑指令
- 拖放文件/文件夹支持
...
| 风险 | 可能性 | 影响 | 缓解措施 |
|------|--------|------|----------|
| 文件系统权限或书签持久化异常 | 低 | 高 | security-scoped bookmark 成熟稳定；提供重新选择文件夹的回退路径 |
| PhotoKit 集成复杂度超出预期（MVP 后） | 中 | 高 | MVP 已验证核心能力；PhotoKit 作为增量来源添加 |
...
| MVP 不支持 Apple 照片图库 | 高 | 中 | 核心价值（AI 管理照片）不依赖来源；文件夹来源覆盖大量用户场景；PhotoKit 作为第二阶段高优先级 |
...
*注：Apple 照片图库（PhotoKit）来源支持计划在 MVP 后第二阶段实现，届时将扩展现有 FR 以覆盖相册、智能相册等 PhotoKit 特有能力。*
